...
describe('sharing the clipboard', () => {
   ...
   * into is one people copy out of by hand.
   */
  it('is on unless something in the chain refuses it', () => {
    expect(resolveRdp({}, null, []).clipboard).toBe(true)
  })

  /**
   * `false` is a value, not an absence, and this is the test that says so: the
   * chain is walked with `firstDefined` rather than `??` for exactly this, and
   * a host that turns the clipboard off must not read as a host that said
   * nothing and inherit its way back to on.
   */
  it('lets a host refuse it', () => {
    expect(resolveRdp({ clipboard: false }, null, []).clipboard).toBe(false)
  })

  it('lets a group refuse it for everything inside', () => {
    const group = { id: 'g', name: 'win', parentId: null, clipboard: false }
    expect(resolveRdp({ groupId: 'g' } as never, 'g', [group]).clipboard).toBe(false)
  })

  it('lets a host allow what its group refused', () => {
    const group = { id: 'g', name: 'win', parentId: null, clipboard: false }
