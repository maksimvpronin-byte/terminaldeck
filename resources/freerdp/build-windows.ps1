...
# What the Visual Studio generator calls the same thing.
...
# No transcript and no trap here, deliberately.
#
# There were both, and between them they hid a failure completely: the first run
# in CI printed the first heading, then the last one, and nothing in between —
# no error, no output from the eight steps that should have run, no clue which
# of them had stopped. Whatever the two of them were doing to the output, a
# build script whose logs cannot say where it stopped is worse than one with no
# logging at all. What is left prints every step as it starts and checks the
# exit code of everything it runs.
function Step([string]$text) { Write-Host "`n==> $text" -ForegroundColor Cyan }
function Ran([string]$what) {
...
if (-not (Test-Path (Join-Path $vcpkg 'vcpkg.exe'))) {
  Step "Fetching vcpkg into $vcpkg"
...
if (-not $ShimOnly) {
  ...
  cmake -S $src -B (Join-Path $src 'build') -G 'Visual Studio 17 2022' -A $vsArch `
    ...
    -DWITH_MANPAGES=OFF
  Ran 'configuring FreeRDP'

  Step 'Building FreeRDP'
  cmake --build (Join-Path $src 'build') --config Release --parallel
  Ran 'building FreeRDP'

  Step "Installing into $out"
  Remove-Item -Recurse -Force $out -ErrorAction SilentlyContinue
  cmake --install (Join-Path $src 'build') --config Release
  Ran 'installing FreeRDP'
}
